                                                        ...
                                                        <tr key={resume.id} className="border-b border-white/5 hover:bg-white/[0.02] transition">
                                                            ...
                                                            <td className="px-6 py-5">
                                                                <div className="flex justify-end gap-2">
                                                                    <a href={contextPath + "/resume-preview?id=" + resume.id} className="p-2 bg-brand-500/20 text-brand-400 rounded-lg hover:bg-brand-500/30 transition shadow-lg group relative" title="Full Preview">
                                                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                                                    </a>

                                                                    {resume.status !== 'APPROVED' && (
                                                                        <a href={contextPath + "/admin/dashboard?action=updateStatus&id=" + resume.id + "&status=APPROVED"} className="p-2 bg-green-500/20 text-green-400 rounded-lg hover:bg-green-500/30 transition shadow-lg" title="Approve Resume">
                                                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                                                                        </a>
                                                                    )}

                                                                    {resume.status !== 'REJECTED' && (
                                                                        <a href={contextPath + "/admin/dashboard?action=updateStatus&id=" + resume.id + "&status=REJECTED"} className="p-2 bg-amber-500/20 text-amber-400 rounded-lg hover:bg-amber-500/30 transition shadow-lg" title="Reject / Draft">
                                                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                                                                        </a>
                                                                    )}

                                                                    <a href={contextPath + "/resume-form?id=" + resume.id} className="p-2 bg-emerald-500/20 text-emerald-400 rounded-lg hover:bg-emerald-500/30 transition shadow-lg" title="Administrative Edit">
                                                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                                                    </a>
                                                                    <button onClick={() => handleDelete(resume.id)} className="p-2 bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 transition shadow-lg" title="Permanent Delete">
